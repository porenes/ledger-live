...
class EventEmitter private constructor
    ...
    fun dispatch(payload: WritableMap) {
        ...
        if (queuedEvents.isNotEmpty()) {
            ...
            if (
                ...
                queuedEvents.set(queuedEvents.count() - 1, payload)
                consumeEventQueue()
                return
            }
        }
    ...
    private fun consumeEventQueue() {
        if (!isJavaScriptAvailable || isConsumingQueue) {
            Timber.tag("BleTransport").e("Already consuming or JS unavailable")
            return
        }
        ...
        while (queuedEvents.isNotEmpty() && isJavaScriptAvailable) {
            ...
            val lambdaExec: (event: WritableMap) -> Unit = {
                ...
                Timber.tag("BleTransport").e("emitting event ($it)")
                context
            ...
            if (pendingEvent != null && lastEventType == event.getString("type")) {
                Timber.tag("BleTransport").e("Cancel the scheduled task, schedule a new one with the new lambda")
                ...
                Timber.tag("BleTransport").e("Scheduling the event, it's too soon")
                pendingEvent = Timer()
